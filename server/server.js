...
const aprilFoolsEffects = {
    troutPopulation: false,
    randomChimpEvent: false,
    microslopStocks: false,
    meteors: false,
    snowstorm: false,
}
    ...
    (async () => {
        ...
        const spawnBot = (loc = null) => {
            ...
            let tank = c.serverName === "Infiltration" ? Class[ran.choose(["infiltrator", "infiltratorFortress", "infiltratorTurrates"])] : ran.choose(botTanks),
                ...
                ]) : ran.choose([ // Dupes act as a weight system lol
                    [0, 0, 4, 8, 8, 9, 8, 5, 0, 0],
                    [0, 0, 5, 9, 9, 9, 9, 1, 0, 0],
                    [0, 0, 8, 7, 7, 8, 5, 7, 0, 0],
                    [2, 4, 2, 7, 6, 9, 6, 5, 0, 1],
        ...
        ioTypes.flee = class extends IO {
            constructor(b) {
                super(b);
            }
            think(input) {
                if (input.fire && input.target != null) return {
                    goal: {
        ...
        class Entity {
            ...
            life() {
                ...
                if (this.onTick) this.onTick(this, sockets);
            }
            addController(newIO) {
        ...
        // APRIL FOOLS 2026 MESSAGE LOOP
        const aprilFoolsMessages = [
            "BREAKING NEWS: Mojang finally gets their shit together",
            "BREAKING NEWS: Arrowhead Studios finally listens to the community",
            "THIS JUST IN: Woomy Update Finally Releases",
            "BREAKING NEWS: Woomy gets acquired by Microsolp",
            "THE END?: Microslop set to sunset Woomy April 1st, 2026",
            "BRIGHTER HORIZONS: Microslop unifies 2D tank games with OSA https://github.com/AE0hello/open-source-arras",
            "DARKER HORIZONS: Microslop forces AI into OSA",
            "WATCH YOUR HEAD: Man trains pitbull to drop bricks on people who ring doorbell",
            "EXCLUSIVE HELLCAT INTERVIEW: \"K\"",
            "COMMUNITY POLL: We asked our audiance what they think of the recent events. One user said \"Yeah, I saw this coming, this is what? the 9th time Woomy has been deleted?\"",
            "COMMUNITY POLL: We asked our audiance once again what they think, \"I don't know why people are surprised, Microslop has been doing this kind of thing for years.\"",
            "BREAKING NEWS: Another Woomy shutdown before GTA 6",
            "EXCLUSIVE CATS INTERVIEW: All your Woomys are belong to us",
            "NEW MICROSLOP STATEMENT: Copilot for OSA, Coming Soon™",
            "ORDER FROM THE TOP: Send 4ourela funny cat pictures on discord",
            "Join us in the news room! https://discord.gg/rqBfa6tYMm",
            "Listen in on our sister channel! https://discord.gg/zCpBTMjsfR",
            "EXCLUSIVE MISFIT INTERVIEW: \"Kys\"",
            "EXCLUSIVE MISFIT INTERVIEW: \"what, i aint even do nothin 🥀\"",
            "EXCLUSIVE VIOLET INTERVIEW: \"I will kill you, with a potato chip\"",
            "EXCLUSIVE 千本桜 INTERVIEW: \"I'm was busy doing things\"",
            "EXCLUSIVE COMET INTERVIEW: \"I am tired and must to sleep soon\"",
            "LIVE WATCH: Yup, the world is still ending",
            "LIVE UPDATE: Diep.io kills another beloved game!",
            "SPECIAL INSIGHT: Earlier today we got an exclusive look into the Woomy boardroom, \"Yeah, we're fucked\" said Drako Hyena",
            "NEW REPORT: Woomy's next update is only month(s) away!",
            "URGENT NEWS: Woomy faces severe data leak! Every detail about users leaked!",
            "DARKER HORIZONS: OSA Development team replaced with AI",
            "TECH NEWS: Woomy requires even more RAM now!",
            "TECHNICAL INSIGHT: After a long meeting with Drako Hyena we have officially determined that, the codebase is infact shit",
            "MICROSLOP INNOVATION: WoomyCoin launched, get in before it gets big!",
            "URGENT INVESTOR NEWS: WoomyCoin rug pulled! All value has been lost!",
            "DEVELOPMENT UPDATE: All remaining bugs are now deemed as features",
            "SERVER STATUS: Nah, this shit ain't lasting",
            "PERFORMANCE REPORT: Ping increased by 2000ms",
            "THIS JUST IN: You can now press crtl + w for developer powers",
            "LOCAL TRAGEDY: Level 10 Basic killed in a driveby from a level 60 Booster",
            "COMING SOON: Microslop will be adding kernal-level anti-cheat to OSA",
            "THIS JUST IN: OSA and Woomy will soon be exclusive to windows!",
            "POLITICAL UPDATE: Tensions rise between teams Blue and Red",
            "DOCUMENTARY PREVIEW: 'The Life of a Crasher: Spawning, Dying, and Repeating' - coming this fall to theaters.",
            "SPECIAL REPORT: Does the game hate you personally? Our data scientists say 'yes, absolutely'.",
            "GLOBAL VIEW: Discourse online has been centered around the meteor strikes. Some conspiracy theoriest claim Aliens.",
            "STREET VIEW: On the street with a live report, 'Why the fuck is it snowing'",
            "STATUS QUO: Developer begs users to learn to code and to start modding",
            "FACT OR FICTION: Do Shinies Exist?",
            "BREAKING NEWS: Recent data analysis shows that your build is trash",
            "JOB OPENING: Microslop is hiring developers to work on OSA!",
            "DATA LEAK: Microslop accidentally leaks all Woomy users' bank cards"
        ]
        setInterval(() => {
            sockets.talkToAll("danews", ran.choose(aprilFoolsMessages))
        }, 45000)
        ...
        const maintainLoop = (() => {
            ...
            const troutPopulation = (() => {
                const spawnTrout = () => {
                    let times = (Math.random() * 10 | 0) + 1;
                    const pos = {x: Math.random() * room.width, y: Math.random() * room.height};
                    for (let i = 0; i < times; i++) {
                        let o = new Entity(pos);
                        o.define(Class.trout, ran.chance(c.SHINY_CHANCE) ? { isShiny: true } : {});
                        o.roomLayerless = true;
                        o.team = -100;
                        o.dangerValue = 0;
                        o.sandboxId = room.id;
                ...
                return function () {
                    if (aprilFoolsEffects.troutPopulation === false) return;
                    if (Math.random() < aprilFoolsConfig.troutSpawnChance) spawnTrout();
                }
            })();
            const chimpEvent = (() => {
                const spawnChimp = () => {
                    let times = (Math.random() * 5 | 0) + 1;
                    const pos = {x: Math.random() * room.width, y: Math.random() * room.height};
                    for (let i = 0; i < times; i++) {
                        let o = new Entity(pos);
                        o.define(Class[`chimp${Math.floor(Math.random()*3)+1}`], ran.chance(c.SHINY_CHANCE) ? { isShiny: true } : {});
                        o.roomLayerless = true;
                        o.team = -101;
                        o.dangerValue = 0;
                        o.sandboxId = room.id;
                        o.facing = ran.randomAngle();
                        o.skill.set([4, 4, 2, 7, 7, 7, 3, 8, 0, 0])
                    }
                };
                return function () {
                    if (aprilFoolsEffects.randomChimpEvent === false) return;
                    if (Math.random() < aprilFoolsConfig.chimpSpawnChance) spawnChimp();
                }
            })();
            const microslopShares = (() => {
                function spawnShare() {
                    let times = (Math.random() * 20 | 0) + 1;
                    const pos = {x: Math.random() * room.width, y: Math.random() * room.height};
                    for (let i = 0; i < times; i++) {
                        let o = new Entity(pos);
                        o.define(Class.microslopShare, ran.chance(c.SHINY_CHANCE) ? { isShiny: true } : {});
                        o.roomLayerless = true;
                        o.team = -100;
                        o.dangerValue = 0;
                        o.sandboxId = room.id;
                ...
                return function () {
                    if (aprilFoolsEffects.microslopStocks === false) return;
                    if (Math.random() < aprilFoolsConfig.shareSpawnChance) spawnShare();
                }
            })();
            const makeMeteors = (() => {
                function spawnMeteors() {
                    let times = (Math.random() * 8 | 0) + 3;
                    let bombardRange = 900;
                    const pos = {x: Math.random() * room.width, y: Math.random() * room.height};
                    for (let i = 0; i < times; i++) {
                        setTimeout(()=>{
                            let o = new Entity({
                                x: pos.x + Math.floor(Math.random() * bombardRange) - bombardRange/2,
                                y: pos.y + Math.floor(Math.random() * bombardRange) - bombardRange/2
                            });
                            o.define(Class.meteorinsignia, ran.chance(c.SHINY_CHANCE) ? { isShiny: true } : {});
                            o.roomLayerless = true;
                            o.team = -102;
                            o.dangerValue = 0;
                            o.sandboxId = room.id;
                ...
                return function () {
                    if (aprilFoolsEffects.meteors === false) return;
                    if (Math.random() < aprilFoolsConfig.meteorSpawnChance) spawnMeteors();
                }
            })();
            const doSnowstorm = (() => {
                function spawnSnow() {
                    let times = (Math.random() * 250 | 0) + 250;
                    let bombardRange = 2000;
                    const pos = {x: Math.random() * room.width, y: Math.random() * room.height};
                    for (let i = 0; i < times; i++) {
                        setTimeout(()=>{
                            let o = new Entity({
                            ...
                            o.define(Class.snowstormFlake, ran.chance(c.SHINY_CHANCE) ? { isShiny: true } : {});
                            o.roomLayerless = true;
                            o.team = -102;
                            o.dangerValue = 0;
                            o.sandboxId = room.id;
                ...
                return function () {
                    if (aprilFoolsEffects.snowstorm === false) return;
                    spawnSnow();
                }
            })();
            return () => {
        ...
        function aprilFoolsEffectsFunc() {
            const effect = Math.floor(Math.random() * 5);
            console.log("Triggering April Fools Effect: " + effect)
            switch (effect) {
                case 0:
                    aprilFoolsEffects.troutPopulation = true;
                    sockets.talkToAll("danews", "BREAKING NEWS: The trout population has sky rocketed!");
                    setTimeout(() => {
                        sockets.talkToAll("danews", "BREAKING NEWS: OH GOD! THE TROUT POPULATION IS CRUMBLING!");
                        aprilFoolsEffects.troutPopulation = false
                    }, aprilFoolsConfig.troutDur);
                    break;
                case 1:
                    aprilFoolsEffects.randomChimpEvent = true;
                    sockets.talkToAll("danews", "CATACLYSMIC EVENT: CHIMPCON 1. I REPEAT, CHIMPCON 1. THE RANDOM CHIMP EVENT IS HAPPENING. FEND FOR YOUR LIVES.");
                    setTimeout(() => {
                        sockets.talkToAll("danews", "TEMPORARY RELIEF?: The chimps have ceased their escapade... for now...");
                        aprilFoolsEffects.randomChimpEvent = false
                    }, aprilFoolsConfig.chimpDur);
                    break;
                case 2:
                    aprilFoolsEffects.microslopStocks = true;
                    sockets.talkToAll("danews", "THIS JUST IN: Microsolp stocks are sky rocketing!");
                    setTimeout(() => {
                        sockets.talkToAll("danews", "MARKET CRASH: Microsolp stocks have plummeted! Experts are baffled!");
                        aprilFoolsEffects.microslopStocks = false
                    }, aprilFoolsConfig.stocksDur);
                    break;
                case 3:
                    aprilFoolsEffects.meteors = true;
                    sockets.talkToAll("danews", "ALIENS?: An unprecedented meteor shower is approaching!");
                    setTimeout(() => {
                        sockets.talkToAll("danews", "NEWS FROM NASA: From an interview with the NASA director, \"No, aliens do not exist. Trust us bro. It was totally just space rocks. Anyways, that stuff should be ending soon.\"");
                        aprilFoolsEffects.meteors = false
                    }, aprilFoolsConfig.meteorDur);
                    break;
                case 4:
                    aprilFoolsEffects.snowstorm = true;
                    sockets.talkToAll("danews", "WEATHER ALERT: Unexpected temperature changes have been seen across the arena!");
                    setTimeout(() => {
                        sockets.talkToAll("danews", "WEATHER UPDATE: The snow has ceased");
                        aprilFoolsEffects.snowstorm = false
                    }, aprilFoolsConfig.snowstormDur);
                break;
            }
        }
        setInterval(aprilFoolsEffectsFunc, aprilFoolsConfig.effectInterval);
        setTimeout(aprilFoolsEffectsFunc, 3000);
