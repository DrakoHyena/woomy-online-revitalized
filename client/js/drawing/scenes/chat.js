...
chat.utilityFuncts.set("enterToChat", () => {
    ...
    if (enterDown && Date.now() - lastEnterPress > chatEnterDebounce) {
        ...
        if (isInputFocused("chatInput")) {
            ...
            if (!state.active) {
                openChatMenu();
                state.openedByEnter = true;
            }
            forceFocusInput("chatInput");
        }
    }
});
...
function draw({ canvas, ctx, delta }) {
    ...
    (function updatePopupHeight() {
        ...
        const border = baseSize * 0.01 * borderMult * scale;
    ...
    const scale = currentSettings.chatSize
    ...
    const borderMult = currentSettings.chatBorderSize
    ...
    state.x =
        (canvas.width - WIDTH - margin) * state.fade +
        canvas.width * (1 - state.fade);
    ...
    renderInput(
        ...
        "text",
        x + border,
        y + border + baseHeight * boxInputRatio + border / 2,
        baseWidth,
        baseHeight * (1 - boxInputRatio),
        "Type here to chat",
        (val) => {
            ...
            for (let i = 0; i < val.length; i += lengthLimit) {
                socket.send(
                    clientPackets.chatMessage,
                    val.substring(i, lengthLimit)
                );
            }
            lastEnterPress = Date.now();
    ...
    const textSize = baseHeight * (1 - boxInputRatio)
    ctx.save();
    let path = new Path2D();
    ...
    if (click.left === true) {
        mouse.scrollY += (lastMouseY - mouse.y) * 0.44;
    }
    if (click) yOffset -= mouse.scrollY * 0.3;
    if (lowestY - (cy + height) > 0) {
        yOffset -= 1 + (lowestY - (cy + height)) * 0.03;
    } else if (Math.ceil(lowestY - (cy + height)) !== 0) {
        yOffset += 1 + (cy + height - lowestY) * 0.03;
    }
    lastMouseY = mouse.y;
    cy += yOffset;

    for (let chatMessage of roomState.chatMessages) {
        const chatMessageFade = Math.min(
            1,
            (performance.now() - chatMessage.creationStamp) /
            Math.max(
                1,
                500 * (1 - currentSettings.menuAnimSpeed.value.number)
            )
        );
        ctx.globalAlpha =
        ...
        const senderText = chatMessage.getSenderRender(
            textSize / 2.5,
            maxWidth
        );
        const contentText = chatMessage.getContentRender(
            textSize / 2,
            maxWidth
        );

        const totalHeight =
            senderText.height + contentText.height + state.messagePadding;
        const layoutIncrement =
            state.messagePadding / 2 +
            2 +
            (senderText.height + contentText.height) * chatMessageFade;

        const clipTop = y
        const clipBottom = clipTop + height;
        const msgTop = cy;
        ...
        if (click) {
            ...
            chatMessage.alphaFade = lerp(
                chatMessage.alphaFade,
                1.25,
                currentSettings.menuAnimSpeed.value.number * delta
            );
            if (click.left) {
...
function drawSelectedMessage(canvas, ctx, delta) {
    ...
    if (selectedMessage.active) {
        selectedMessage.fade = lerp(
            selectedMessage.fade,
            1,
            currentSettings.menuAnimSpeed.value.number * delta
        );
    } else {
    ...
    ctx.globalAlpha =
        ...
        selectedMessage.fade;
    ctx.fillStyle = ACCENT;
    ctx.fillRect(x, y, width - state.margin, height);
    ctx.fillStyle = BACKGROUND;
    // calculate inner content area (without border)
    const innerW = baseWidth
    const innerH = baseHeight
    ctx.fillRect(x + border, y + border, innerW, innerH);

    const boxSize = height / 1.5;
    const marg = state.margin * scale;
    ctx.fillStyle = "#E02121";
    let click = clickableActive(
        x,
        y + height / 2 - boxSize / 2,
        x + boxSize,
        y + height / 2 - boxSize / 2 + boxSize
    );
    if (click) {
        ctx.globalAlpha *= 1.3;
    ...
    if (click) {
        ...
        if (mutedSenders.has(selectedMessage.author)) {
            showCursorTextBox(
                "Unmute Sender",
                "Allows all incoming messages from this user in the global chat for the rest of the session"
            );
        } else {
            showCursorTextBox(
                "Mute Sender",
                "Blocks all incoming messages from this user in the global chat for the rest of the session"
            );
        }
        hadMuteFocus = true;
        if (click.left && Date.now() - lastClick > clickDebounce) {
...
function drawNewChats({ canvas, ctx, delta }) {
    ...
    const fadeTime = Math.max(
        1,
        400 * (1 - currentSettings.menuAnimSpeed.value.number)
    );
    ...
    for (let i = roomState.chatMessages.length - 1; i >= 0; i--) {
        ...
        if (tDiff < fadeTime) {
            ctx.globalAlpha = Math.min(
                newChatAlpha,
                (newChatAlpha * tDiff) / fadeTime
            );
        } else if (tDiff > maxDur - fadeTime) {
            ctx.globalAlpha = Math.max(
                0,
                (newChatAlpha * (maxDur - tDiff)) / fadeTime
            );
        } else {
        ...
        const senderText = chatMessage.getSenderRender(
            (canvas.height / 60) * currentSettings.chatOverlaySize.value.number,
            maxWidth
        );
        const contentText = chatMessage.getContentRender(
            (canvas.height / 60) * currentSettings.chatOverlaySize.value.number,
        ...
        const contentX = rectLeft + maxWidth - contentText.width;
        ...
        const bgLeft = senderX;
        const bgRight = senderX + senderText.width + contentText.width;
