...
function drawNameplate({ canvas, ctx, delta }) {
    ...
    drawBar(ctx, x, y, contentWidth, baseTextSize / 3, 1, "lightgreen", "black", playerState.levelProgress, currentSettings.squareNameplateBars.value.enabled ? "square" : "round")
...
let lastLinkStat = false;
nameplate.drawFuncts.set("fade", ({ canvas, ctx, delta }) => {
    const ANIM_SPEED = currentSettings.menuAnimSpeed.value.number * delta

    if (lastLinkStat !== currentSettings.linkNameplateToStats.value.enabled) {
        state.active = true;
    }

    if (currentSettings.linkNameplateToStats.value.enabled) {
        state.fade = lerp(state.fade, statMenuState.fade, ANIM_SPEED);
    ...
    lastLinkStat = currentSettings.linkNameplateToStats.value.enabled
})
