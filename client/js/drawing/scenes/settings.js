...
settings.drawFuncts.set("settingsMenu", ({ canvas, ctx, delta }) => {
    ...
    text = renderText("Chat Overlay Size", SETTING_TEXT_SIZE);
    if ((y - text.height) < height && y > 0) {
        ctx.drawImage(text, x + SETTINGS_CONFIG.PADDING, y);
        renderInput(
            "chatOverlaySize",
            ...
            text.height * 3,
            text.height,
            currentSettings.chatOverlaySize.value.number,
            (newNumber) => {
                currentSettings.chatOverlaySize.value.number = newNumber;
            },
            () => {
                showCursorTextBox(
                    "Chat Overlay Size",
                    "A size multiplier applied to the chat overlay chat UI. (0.1 - 5.0)"
                );
            },
            hideCursorTextBox
        );
    ...
    text = renderText("Chat Overlay Opacity", SETTING_TEXT_SIZE);
    if ((y - text.height) < height && y > 0) {
        ctx.drawImage(text, x + SETTINGS_CONFIG.PADDING, y);
        renderInput(
            "chatOverlayAlpha",
            ...
            text.height * 3,
            text.height,
            currentSettings.chatOverlayAlpha.value.number,
            (newNumber) => {
                currentSettings.chatOverlayAlpha.value.number = newNumber;
            },
            () => {
                showCursorTextBox(
                    "Chat Overlay Opacity",
                    "Controls the overall opacity of the overlay chat UI (0.0 - 2.0)"
                );
            },
            hideCursorTextBox
        );
    ...
    text = renderText("Delta Maximum", SETTING_TEXT_SIZE);
    if ((y - text.height) < height && y > 0) {
        ctx.drawImage(text, x + SETTINGS_CONFIG.PADDING, y);
        renderInput(
            "deltaMax",
            ...
            text.height * 3,
            text.height,
            currentSettings.deltaMax.value.number,
            (newNumber) => {
                currentSettings.deltaMax.value.number = newNumber;
            }, () => {
                showCursorTextBox("Delta Maximum", "How much the client is able to compensate for lost frames (0.1-5). Warning: This setting maybe result in broken or flashing visuals if set too low or high.");
            },
            hideCursorTextBox,
        );
    }
