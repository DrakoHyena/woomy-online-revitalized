...
document.addEventListener("keydown", function eh(e) {
	...
	if (document.getElementById("gameJoinScreen").style.zIndex !== "101")
		return;
...
const defaultGamemodes = [
	...
	},
	{
		...
		description:
			"Kill other players on the other team to recruit them to your team.",
		players: 0,
		code: "2tag.json"
	},
	{
	...
	},
	{
		...
		description:
			"Fight on one of two teams to kill the other team's mothership: Now with portals!",
		players: 0,
		code: "p2mot.json"
	},
	{
		...
		description:
			"Fight against the other team's army with your own! Each side has 25 bots.",
		players: 0,
		code: "2tdmhell.json"
	},
	{
	...
	},
	{
		...
		description:
			"Fight in an all out war against three other teams while in the dark...",
		players: 0,
	...
	},
	{
		...
		description:
			"Each player has their own arena. Test different combos here.",
		players: 0,
		code: "sbx.json"
	},
	{
	...
	},
	{
		...
		description:
			"Travel into the beyond, past the boarders of the map. Beware the danger entities that lie far out.",
		players: 0,
		code: "vwalk.js"
	},
	{
...
async function showRooms() {
	...
	for (let room of rooms) {
		...
		ele.children[0].children[0].innerText =
			gamemodeInfo.name || gamemodeInfo;
	...
	if (createFilter === "host") {
		...
		for (let ele of roomEles) {
			if (
				ele.children[0].children[0].innerText
					.toLowerCase()
					.includes(term) === false && // Gamemode Name
				ele.children[0].children[0].innerText
					.toLowerCase()
					.replaceAll(" ", "")
					.includes(term) === false &&
				ele.children[0].children[1].innerText
					.toLowerCase()
					.includes(term) === false && // Player Count
				ele.children[0].children[1].innerText
					.toLowerCase()
					.replaceAll(" ", "")
					.includes(term) === false &&
